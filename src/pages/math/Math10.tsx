...
function Math10() {
  ...
  const generateRandomNumbers = () => {
    ...
    while (incorrectAnswers.length < 2) {
      ...
      if (randomIncorrect !== sum && !incorrectAnswers.includes(randomIncorrect)) {
        incorrectAnswers.push(randomIncorrect)
      }
    }

    const answerChoices = [...incorrectAnswers, sum].sort(() => Math.random() - 0.5)
    ...
    <div>
      ...
      {selectedAnswer !== null && (
        <p>
          ...
          {selectedAnswer ? (selectedAnswer === correctAnswer ? '(Correct)' : '(Incorrect)') : ''}
        </p>
      )}
